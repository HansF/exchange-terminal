...
const api = {
  ...
  startDay: (): Promise<DayState> =>
    fetch('/api/day/start', { method: 'POST' }).then((r) => r.json()),
  ...
  return (
    <>
      ...
      >
        ...
        <button
          type="submit"
          className="border-2 border-black bg-black text-[#FFE135] px-4 py-2 font-black text-sm"
        >
          +
        </button>
      </form>
...
export default function Todo({ onBack }: Props) {
  ...
  useEffect(
    () => () => {
      [intervalRef, breakIntervalRef, copyIntervalRef].forEach((r) => {
  ...
  if (day === 'loading') {
    return (
      <div className="min-h-screen bg-[#FFE135] flex items-center justify-center font-mono text-2xl font-black animate-pulse">
        …
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-[#FFE135] font-mono flex flex-col">
      ...
      {!dayActive && !dayEnded && timerPhase === 'idle' && (
        <div className="min-h-screen flex flex-col items-center justify-center gap-8 p-8">
          ...
          <button
            onClick={onBack}
            className="text-sm font-bold text-black/40 hover:text-black underline transition-colors flex items-center gap-1"
          >
        ...
        <div className="min-h-screen flex flex-col items-center justify-center gap-6 p-8">
          <div className="text-center">
            ...
            <p className="text-sm font-bold text-black/50 mt-1">
              {todaySessions.length} sessions · {totalMin} min
            </p>
          </div>
          <button
            onClick={resetDay}
            className="text-sm font-bold text-black/40 hover:text-black underline transition-colors"
          >
            Reset & start fresh
          </button>
          <button
      ...
      {dayActive && timerPhase === 'idle' && (
        <div className="flex flex-col min-h-screen p-6 max-w-lg mx-auto w-full">
          ...
          <div className="flex items-center gap-3 mb-8">
            <button
              onClick={onBack}
              className="bg-white border-2 border-black font-bold px-3 py-2 shadow-[3px_3px_0px_0px_rgba(0,0,0,1)] hover:-translate-y-0.5 hover:shadow-[5px_5px_0px_0px_rgba(0,0,0,1)] active:shadow-none transition-all flex items-center gap-1 text-sm"
            >
            ...
            <button
              onClick={resetDay}
              className="text-xs font-black uppercase tracking-wide text-black/30 hover:text-black border border-black/20 px-2 py-1 hover:border-black transition-colors"
            >
              Reset
            </button>
          </div>
          ...
          <div className="flex-1 flex flex-col gap-4 justify-center">
            ...
            {!showRetro ? (
              <button
                ...
                className="text-sm font-bold text-black/40 hover:text-black transition-colors underline underline-offset-2 self-center"
              >
                + log a past session
              </button>
            ) : (
              <div className="bg-white border-4 border-black shadow-[6px_6px_0px_0px_rgba(0,0,0,1)] p-4 flex flex-col gap-4">
                <div className="flex items-center justify-between">
                  ...
                  <button
                    ...
                    className="text-xs font-bold text-black/40 hover:text-black uppercase"
                  >
                    cancel
                  </button>
                </div>
                <div className="flex items-center gap-3">
                  <label className="text-xs font-black uppercase tracking-widest text-black/50 shrink-0">
                    Duration
                  </label>
                  <input
                    type="number"
                ...
                <TagPicker
                  ...
                  onAddCustom={() => {
                    const t = retroTagInput.trim().toLowerCase();
                    if (t) {
                      setRetroTags((prev) => new Set([...prev, t]));
                      setRetroTagInput('');
                    }
                  }}
                />
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex gap-2">
                    {([1, 2, 3, 4, 5] as const).map((n) => (
                      <button
                        key={n}
                        onClick={() => setRetroRating((prev) => (prev === n ? null : n))}
                        className={`text-xl transition-all hover:scale-125 ${retroRating !== null && n <= retroRating ? 'opacity-100' : 'opacity-20'}`}
                      >
                        ★
                      </button>
                    ))}
            ...
            <div className="mt-8 border-t-2 border-black pt-4 flex flex-col gap-2">
              {todaySessions.map((s, i) => (
                <div key={s.id} className="flex items-baseline gap-3 text-sm">
                  <span className="text-black/30 font-bold shrink-0 text-xs w-4 text-right">
                    {i + 1}
                  </span>
                  <span className="text-black/40 font-bold shrink-0 text-xs">
                    {timeStr(s.startedAt)}
                  </span>
                  ...
                  <span className="flex-1 flex flex-wrap gap-1 min-w-0">
                    {s.tags.length > 0 ? (
                      s.tags.map((tag) => (
                        <span
                          key={tag}
                          className="text-[10px] font-black uppercase bg-black text-[#FFE135] px-1.5 py-0.5"
                        >
                          {tag}
                        </span>
                      ))
                  ...
                  {s.wasAbandoned && (
                    <span className="text-[9px] font-black text-black/30 shrink-0">PART.</span>
                  )}
                </div>
              ))}
            </div>
            ...
            {printError && (
              <div className="border-2 border-black bg-red-100 px-3 py-2 text-xs font-bold break-all text-red-700 flex items-start gap-2">
                ...
                <button
                  ...
                  className="ml-auto shrink-0 font-black hover:text-black"
                >
                  ✕
                </button>
              </div>
      ...
      {(timerPhase === 'running' || timerPhase === 'paused') && (
        <div className="min-h-screen flex flex-col items-center justify-center gap-10 p-8 select-none">
          ...
          <div
            className={`text-[clamp(6rem,20vw,10rem)] font-black leading-none tabular-nums transition-opacity ${timerPhase === 'paused' ? 'opacity-20' : ''}`}
          >
            {fmt(secondsLeft)}
          </div>
          ...
          <div className="flex gap-4">
            {timerPhase === 'running' ? (
              <button
                ...
                className="bg-white border-4 border-black font-black px-8 py-4 shadow-[6px_6px_0px_0px_rgba(0,0,0,1)] hover:-translate-y-1 hover:shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] active:translate-y-0.5 active:shadow-none transition-all flex items-center gap-2 uppercase"
              >
              ...
              <button
                ...
                className="bg-black text-[#FFE135] border-4 border-black font-black px-8 py-4 shadow-[6px_6px_0px_0px_rgba(0,0,0,1)] hover:-translate-y-1 active:translate-y-0.5 active:shadow-none transition-all flex items-center gap-2 uppercase"
              >
            ...
            <button
              onClick={endEarly}
              className="border-2 border-black/30 bg-white font-bold px-5 py-4 text-sm uppercase text-black/40 hover:text-black hover:border-black transition-all flex items-center gap-2"
            >
      ...
      {timerPhase === 'break' && (
        <div className="min-h-screen bg-[#E0FF4F] flex flex-col items-center justify-center gap-6 p-8">
          ...
          <button
            ...
            className="border-2 border-black bg-white font-black px-6 py-2 text-sm uppercase hover:-translate-y-0.5 transition-all"
          >
            Skip
          </button>
        </div>
        ...
        <div className="fixed inset-0 bg-[#FF90E8] flex flex-col overflow-y-auto z-50">
          <div className="max-w-md mx-auto w-full p-6 flex flex-col gap-6 py-10">
              ...
              <div className="flex gap-3">
                {([1, 2, 3, 4, 5] as const).map((n) => (
                  <button
                    ...
                    className={`text-3xl transition-all hover:scale-125 ${rating !== null && n <= rating ? 'opacity-100' : 'opacity-20'}`}
                  >
                    ★
                  </button>
                ))}
            ...
            <div className="flex flex-col gap-3 pt-1">
              ...
              {printError && (
                <div className="bg-white border-2 border-black px-3 py-2 text-xs font-bold break-all text-red-700">
                  ⚠ {printError}
                </div>
              )}
            </div>
...
function DayHeader({ day }: { day: DayState | null }) {
  ...
  return (
    <div style={{ paddingBottom: 12 }}>
      <div style={{ ...T.center, ...T.xs, ...T.bold, letterSpacing: '0.3em', marginBottom: 6 }}>
        ✦ ✦ ✦
      </div>
      <div
        style={{
      ...
      <div style={{ borderTop: '1px solid #000', paddingTop: 8, ...T.center }}>
        <div style={{ ...T.xs, ...T.bold, textTransform: 'uppercase', letterSpacing: '0.2em' }}>
          Day started at {time}
        </div>
      </div>
    </div>
  ...
  return (
    <div style={{ borderTop: '1px dashed #cccccc', paddingTop: 8, paddingBottom: 8, fontSize: 11 }}>
      <div style={{ ...T.row, ...T.bold, marginBottom: 3 }}>
        <span>
          #{num} {timeStr(session.startedAt)}
        </span>
        <span>
          {fmt(session.durationSeconds)}
          {session.wasAbandoned ? ' part.' : ''}
        </span>
      </div>
      ...
      <div style={{ borderTop: '1px solid #000', paddingTop: 12, marginTop: 4 }}>
        ...
        {sessions.map((s, i) => (
          <div key={s.id} style={{ marginBottom: 8, fontSize: 11 }}>
            ...
            {s.tags.length > 0 && (
              <div
                style={{ ...T.xs, ...T.muted, textTransform: 'uppercase', letterSpacing: '0.05em' }}
              >
                {s.tags.join(' · ')}
              </div>
            )}
